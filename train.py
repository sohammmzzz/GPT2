...
class GatedLinearAttentionMC(nn.Module):
    ...
    def forward(self, x):
        ...
        scores = qk * torch.exp(decay).masked_fill(~causal, 0.0)
        ...
        rel = rel.masked_fill(~cross_mask[None, None], float("-inf"))
        gamma = torch.softmax(rel, dim=-1)                             # (B,H,N,N); rows with no past -> ~0
...
class PretrainData:
    ...
    def _hf_stream(self) -> Iterator[int]:
...
def save_ckpt(path, model, opt, stage, step):
    ...
    _atomic_save({"model": model.state_dict(), "opt": opt.state_dict() if opt else None,
                  "stage": stage, "step": step, "cfg": asdict(model.cfg)}, path)
...
def export_weights(path, model):
    ...
    _atomic_save({"model": sd, "cfg": asdict(model.cfg), "tokenizer": "gpt2"}, path)
...
def main():
    ...
    p.add_argument("--finetune_steps", type=int, default=3000)
    ...
    if not args.skip_pretrain:
        ...
        model.load_state_dict(torch.load(pre_ckpt, map_location=device)["model"])
